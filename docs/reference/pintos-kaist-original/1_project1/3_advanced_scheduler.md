...
**Implement a multilevel feedback queue scheduler similar to the [4.4BSD scheduler](#4.4BSD%20Scheduler) to reduce the average response time for running jobs on your system.**
...
> Sets the current thread's nice value to new nice and recalculates the thread's priority based on the new value (see [Calculating Priority](#Calculating%20Priority)). If the running thread no longer has the highest priority, yields.
